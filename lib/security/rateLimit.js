...
// AI/research routes: 30 requests per minute per IP
// "Research All" calls 15 sections sequentially, so we need at least 15/min.
// Set to 30 to allow a "Research All" plus some individual section auto-fills
// within the same minute without hitting the limit.
// Still blocks automated abuse (a bot doing 100+ req/min).
const rateLimitByApiRoute = rateLimit({
  windowMs: 60_000,    // 1 minute
  maxRequests: 30,      // 30 req/min — supports "Research All" (15 sections)
  prefix: 'ai',
});
