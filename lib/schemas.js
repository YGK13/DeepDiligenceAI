...
export function createEmptyCompany(name) {
  return {
    ...
    // ============ NOTES & MEETINGS ============
    // Deal notes and meeting log. VCs take notes during founder calls, IC
    // meetings and back-channel references. These live alongside the DD data.
    // notes: array of { id, text, createdAt }
    // meetings: array of { id, date, type, attendees, notes,
    //   actionItems: [{ id, text, done }], createdAt }
    notes: [],
    meetings: [],

    // ============ DOCUMENTS ============
    // Per-company document vault. Stores metadata for term sheets, cap tables,
    // data room files, board decks, legal docs. Each entry has:
    // { id, name, category, notes, url, size, addedAt }
    documents: [],

    // ============ CONTACTS ============
    // Relationship/contact graph for the deal. VCs don't just track
    // companies — they track PEOPLE: founders, co-investors, board
    // members, advisors, LPs, introducers. Who introduced whom, who's
    // connected to which deal. Stored per-company as an array of
    // contact objects with type, relationship status, and metadata.
    // Each contact: { id, name, title, company, email, phone, linkedin,
    //   type (founder/investor/advisor/board/introducer/other),
    //   relationship (warm/cold/connected/met),
    //   notes, addedAt, lastContactedAt }
    contacts: [],

    // ============ MONITORING CONFIGURATION ============
    // Post-DD ongoing monitoring settings. After initial diligence is complete,
    // VCs need to track portfolio companies for material events: new funding rounds,
    // executive departures, competitor moves, regulatory changes, product launches,
    // and press coverage. Each category can be toggled independently per company.
    // lastChecked tracks when the monitoring engine last ran for freshness display.
    monitoring: {
      enabled: false,                       // Master toggle — is monitoring active for this company?
      categories: {
        funding: true,                      // Track new funding rounds, valuations, investor changes
        leadership: true,                   // Track executive hires, departures, board changes
        competitor: true,                   // Track competitor raises, launches, acquisitions
        regulatory: true,                   // Track lawsuits, compliance issues, regulatory actions
        product: true,                      // Track new features, pivots, product launches
        press: true,                        // Track positive and negative media coverage
      },
      lastChecked: null,                    // ISO timestamp of last monitoring check
    },

    // ============ ALERTS ============
    // Chronological array of monitoring alerts generated by the /api/monitor/check
    // endpoint. Each alert: { id, severity, category, title, description, source,
    // timestamp, read (boolean), dismissed (boolean) }
    // Sorted newest-first in the UI. Unread count drives the nav badge.
    alerts: [],
